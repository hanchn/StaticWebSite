...
class Template {
  ...
  configureEJS() {
    ...
    ejs.cache = options.cache !== false;
    ejs.delimiter = options.delimiter || '%';
    ejs.openDelimiter = options.openDelimiter || '<';
    ejs.closeDelimiter = options.closeDelimiter || '>';
  }
  ...
  async prepareTemplateData(data) {
    const templateData = {
      ...
      // 工具函数
      utils: this.createTemplateUtils(),
  ...
  async getLayout(layoutName) {
    ...
    const layoutPath = path.join(this.layoutPath, layoutName + '.ejs');
